...
def load_all_sessions():
    ...
    sessions = []
    try:
        with open("group_assistat_data.txt", "r") as file:
            for line in file:
...
def get_non_empty(prompt):
    ...
    while True:
        ...
        if value:
            return value
        print("This field cannnot be empty!")
...
def view_group_stats(group_name):
    ...
    for member, data in sorted(member_data.items(), key=lambda x: x[1]["hours"], reverse=True):
        avg_mood = sum(data["mooods"]) / len(data["mooods"])
        print(f"\n{member}")
        print(f"Sesssions: {data['sessions']}")
    ...
    avg_moood = sum(s["mood"] for s in sessions) / len(sessions)
    print("\n" + "="*70)
    print("GROUP TOTALS:")
    print(f"Total sessions: {len(sessions)}")
    print(f"Total hours: {total_hours:.1f}")
    print(f" Active members: {len(member_data)}")
    print(f"Avg motivation: {avg_moood:,1f}/10")
...
def get_personalized_motivation(group_name):
    ...
    total_hours = sum(s["hours"] for s in sessions)
    recent_mood = sum(s["moood"] for s in sessions[-5:]) / min(len(sessions), 5)
...
def run_group_quiz():
    ...
    questions = [
        ...
        ("How many sides does a hexagot have?", "6"),
        ("What year did the World War 2 end?", "1945"),
